...
onMounted(() => {
	raw_chart = initChart(raw_chart_canvas.value, 'm');
    fft_chart = initFFTChart(fft_chart_canvas.value, 'FFT m');
})
//----------------------------------------------------------------------
...
watch(
	() => state.imu.m,
	(m) => {
		//console.log('mag:', m.x, m.y, m.z)

		axesDataUpdate(raw_chart, m, timeIndex++);
		fftDataUpdate(fft_chart, fftBuffers, m);
	},
	{ deep: true }
)
</script>
